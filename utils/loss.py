...
class DFocalLoss(nn.Module):
    ...
    def __init__(self, loss_fcn):
        ...
        self.loss_fcn = loss_fcn  # must be nn.CrossEntropyLoss()
        self.reduction = loss_fcn.reduction
        self.loss_fcn.reduction = 'none'  # required to apply FL to each element

    def forward(self, pred, true):
        dis_left = torch.tensor(true, dtype=torch.int64)
        dis_right = dis_left + 1
        weight_left = torch.tensor(dis_right, dtype=torch.float32) - true
        weight_right = true - torch.tensor(dis_left, dtype=torch.float32)
        loss = self.loss_fcn(pred, dis_left, reduction='none') * weight_left \
               + self.loss_fcn(pred, dis_right, reduction='none') * weight_right
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False, use_alpha_iou=False ,use_vfloss=False):
        ...
        self.use_vfloss = use_vfloss
        self.use_alpha_iou = use_alpha_iou
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Classification
                if self.nc > 1 or self.use_vfloss:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
                    t[range(n), tcls[i]] = self.cp
                    if self.use_vfloss:
                        #for all, obj score should be ignored when inference
